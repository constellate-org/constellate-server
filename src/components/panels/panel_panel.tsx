...
export default class PanelPanel extends React.Component<PanelProps> {
    ...
    loadPlot() {
        let colorMode = this.props.isDark ? "dark" : "light";
        let plot = document.getElementById('imgEmbedContent' + colorMode)
        if (!plot || plot.innerHTML === '') {
            const xhr = new XMLHttpRequest();
            xhr.responseType = 'text';

            let url = this.props.url;
            let path = this.props.star.star_id;
            console.log(`${url}/${path}`);
            xhr.open('GET', `${url}/${path}/autoload.js?bokeh-autoload-element=imgEmbedContent${colorMode}&bokeh-app-path=/${path}&bokeh-absolute-url=${url}/${path}&colorMode=${colorMode}`, true);
            xhr.setRequestHeader("Cache-Control", "no-cache, no-store, max-age=0");
            xhr.setRequestHeader("Bokeh-Session-Id", this.props.uuid + colorMode);
            xhr.setRequestHeader('ColorMode', colorMode);

            let script_id = this.props.uuid + colorMode;
            ...
            xhr.onload = function (event) {
                ...
                if (!document.getElementById(script_id)) {
                    ...
                    script.innerHTML = src.replaceAll(`"static/extensions/panel`, `"${url}/static/extensions/panel`);
    ...
    render() {
        let colorMode = this.props.isDark ? "dark" : "light";
        let invColorMode = !this.props.isDark ? "dark" : "light";
        let tabs = [
            {
                id: 'img',
                name: 'Plot',
            ...
            },
            {
                ...
                content: (
                    <EuiCodeBlock language="python" lineNumbers overflowHeight="100%" fontSize="m" paddingSize="m"
                        isCopyable={true} isVirtualized id="codeBlockEmbed">
        ...
        let embed = document.getElementById(this.props.uuid + invColorMode);
        console.log('3');
        if (embed) {
            console.log('4');
