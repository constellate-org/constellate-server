...
class BetaDist(param.Parameterized):
    ...
    def colors_theme(self):
        if self.colorMode == "light":
            plt.style.use("rho-light.mplstyle")
        else:
            plt.style.use("rho-dark.mplstyle")
        colors = plt.rcParams["axes.prop_cycle"].by_key()["color"]
        theme = bokeh.themes.Theme(f"{self.colorMode}.yml")
        return (colors, theme)
    ...
    def plot(self):
        ...
        p = bokeh.plotting.figure(
            x_range=(0, 1), y_range=(0, max(6, max(yy))), title="Beta Distribution"
        )
...
app = pn.Row(model.param, model.plot, sizing_mode="stretch_both", height=1000)
